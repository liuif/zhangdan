	...
	export default {
		...
		methods: {
			...
			async logstatrt() {
				...
				console.log(data1);
				uni.request({
					url: '/api/getTotalIncomeAndExpenseByYear',
					method: 'POST',
					data: {
						year: uni.getStorageSync("date"),
						email: uni.getStorageSync('email')
					...
					if (result.code === 1) {
						// 登录成功
						const inArray = Array(12).fill(0);
						const outArray = Array(12).fill(0);
						const balArray = Array(12).fill(0);
						...
						result.inandout.forEach(item => {
							...
							inArray[month - 1] = inValue;
							outArray[month - 1] = outValue;
							balArray[month - 1] = balance;
			...
			logstatrtone() {
				...
				uni.request({
					url: '/api/findDailyIncomeExpenseNetByYear',
					method: 'POST',
					data: {
						year: uni.getStorageSync("date"),
						email: uni.getStorageSync('email')
